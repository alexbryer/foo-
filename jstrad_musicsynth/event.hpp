...
private:
	// TODO
	intmax_t tickT;
	//from instructor....need constructor or deconstructor???
	//memset from piazza...check is this is allowed
	enum EventType { endTrackEvent, tempoEvent, noteEvent } eventType;
	union midiunion {
		TempoEvent tempoEvent_;
		EndTrackEvent endTrackEvent_;
		NoteEvent noteEvent_;
		midiunion() { memset(this, 0, sizeof(midiunion)); }
	}event;
};
